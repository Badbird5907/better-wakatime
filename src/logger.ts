  ...
  color: string;
    ...
    _log(level: keyof Console, levelColor: string, args: any[], customFmt = "") {
        if (IS_REPORTER && IS_WEB) {
            (console[level] as Function)("[BetterWakatime]", this.name + ":", ...args);
            return;
        }
        (console[level] as Function)(
            `%c BetterWakatime %c %c ${this.name} ${customFmt}`,
...
export const logger = new Logger("BetterWakatime", "#4175d2");